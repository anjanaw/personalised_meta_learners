...
import numpy as np
import pm_task_generator as tg
import os
import math
import argparse
import random
...
parser.add_argument("-s", "--sample_num_per_class", type=int, default=0)
parser.add_argument("-q", "--qry_num_per_class", type=int, default=1)
...
def main():
    ...
    metatrain_character_folders, metatest_character_folders = tg.pm_folders(TEST_PERSON)
    ...
    for episode in range(EPISODE):
        ...
        task = tg.PMTask(metatrain_character_folders, CLASS_NUM, SAMPLE_NUM_PER_CLASS, qry=QRY)
        ...
        sample_features = sample_features.view(CLASS_NUM, SAMPLE_NUM_PER_CLASS, 64, 18, 2)
        # print('sample_features.shape', sample_features.shape)
        ...
        relation_pairs = torch.cat((sample_features_ext, batch_features_ext), 2).view(-1, 64 * 2, 18, 2)
        ...
        if (episode + 1) % 5 == 0:
            ...
            for i in range(TEST_EPISODE):
                ...
                task = tg.PMTask(metatest_character_folders, CLASS_NUM, SAMPLE_NUM_PER_CLASS)
                ...
                relation_pairs = torch.cat((sample_features_ext, test_features_ext), 2).view(-1, 64*2, 18, 2)
                # print('relation_pairs.shape', relation_pairs.shape)
                relations = relation_network(relation_pairs).view(-1, CLASS_NUM)
                # print('relations.shape', relations.shape)
