...
def dc_folders(test_person):
    data_folder = '/home/mex/data/dc_scaled/0.05_0.05_maml/'
...
class DC(FewShotDataset):

    def __init__(self, *args, **kwargs):
        super(DC, self).__init__(*args, **kwargs)
    ...
    def __getitem__(self, idx):
        ...
        for row in _data:
            ...
            image = np.reshape(image, (5, 12, 16))
            image = np.swapaxes(image, 1, 2)
            image = np.reshape(image, (1, 60, 16))
