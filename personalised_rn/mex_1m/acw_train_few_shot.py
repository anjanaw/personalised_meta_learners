...
import numpy as np
import personalised_rn.mex_1m.acw_task_generator as tg
import os
import math
import argparse
import random
...
def write_results(text):
    file_path = 'acw_few_shot_cce.csv'
...
def main():
    ...
    for episode in range(EPISODE):
        ...
        task = tg.ACWTask(metatrain_character_folders, CLASS_NUM, SAMPLE_NUM_PER_CLASS)
        ...
        sample_features = sample_features.view(CLASS_NUM, SAMPLE_NUM_PER_CLASS, 64, 1, 89)
        # print('sample_features.shape', sample_features.shape)
        ...
        # loss = mse(attentions, one_hot_labels)
        ...
        if (episode + 1) % 5 == 0:
            ...
            for i in range(TEST_EPISODE):
                task = tg.ACWTask(metatest_character_folders, CLASS_NUM, SAMPLE_NUM_PER_CLASS)
