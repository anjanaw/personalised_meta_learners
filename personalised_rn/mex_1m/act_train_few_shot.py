...
import numpy as np
import personalised_rn.mex_1m.act_task_generator as tg
import os
import math
import argparse
import random
...
def write_results(text):
    file_path = 'act_few_shot_cce.csv'
...
def main():
    ...
    for episode in range(EPISODE):
        ...
        task = tg.ACTTask(metatrain_character_folders, CLASS_NUM, SAMPLE_NUM_PER_CLASS)
        ...
        if (episode + 1) % 5 == 0:
            ...
            for i in range(TEST_EPISODE):
                task = tg.ACTTask(metatest_character_folders, CLASS_NUM, SAMPLE_NUM_PER_CLASS)
